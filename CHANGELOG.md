...
- Merge external projectDocu directories into `{worker}/data/projectDocu` before
  build (org theme + project overrides)
- `advanced_doxygenConfig.txt` fragments are concatenated in path order; other
  top-level files use last-wins overwrite
- Public helpers: `mergeProjectDocuSources`, `normalizeProjectDocuPaths`,
  `listProjectDocuFiles`, `substituteDoxygenVersionInFile`
